...
def paired_data_transforms(img_1, img_2, unaligned_transforms=False):
    ...
    target_size = int(random.randint(512, 640) / 2.) * 2
    ...
    i, j, h, w = get_params(img_1, (512,512))
...
class CEILDataset(BaseDataset):
    def __init__(self, datadir, fns=None, size=None, enable_transforms=True, low_sigma=2, high_sigma=5, low_gamma=1.3, high_gamma=1.3):
        ...
        B_datadir = os.path.join(datadir, 'transmission_layer/')
        R_datadir = os.path.join(datadir, 'reflection_layer/')
        self.B_paths = sorted(make_dataset(B_datadir, fns), key=sortkey)
        self.R_paths = sorted(make_dataset(R_datadir, fns), key=sortkey)
        if size is not None:
